...
class COCOStuffeval:
    ...
    def __init__(self, cocoGt, cocoRes, stuffStartId=92, stuffEndId=182, addOther=True):
        ...
        self.params.imgIds = sorted(
            cocoGt.getImgIds()
        )  # By default we use all images from the GT file
        self.catIds = range(
            stuffStartId, stuffEndId + addOther + 1
        )  # Take into account all stuff
    ...
    def evaluate(self):
        ...
        print(
            "Evaluating stuff segmentation on %d images and %d classes..."
        ...
        for i, imgId in enumerate(imgIds):
            ...
            confusion = self._accumulateConfusion(
                self.cocoGt, self.cocoRes, confusion, imgId
            )
    ...
    def summarize(self):
        ...
        [miouSup, fwiouSup, maccSup, paccSup, iousSup, maccsSup] = self._computeMetrics(
            confusionSup
        )
    ...
    def _getSupCatConfusion(self, confusion):
        ...
        for supCatIdA in xrange(0, supCatCount):
            for supCatIdB in xrange(0, supCatCount):
                curLeavesA = (
                    np.where([s == supCatIdA for s in supCatIds])[0]
                    + self.stuffStartId
                    - 1
                )
                curLeavesB = (
                    np.where([s == supCatIdB for s in supCatIds])[0]
                    + self.stuffStartId
                    - 1
                )
